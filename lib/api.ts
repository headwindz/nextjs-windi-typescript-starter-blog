...
const getFullPathWithLocale = (dirPath: string, locale?: string) => {
  const fileName = locale && locale !== themeConfig?.i18n?.defaultLocale ? `index.${locale}.md` : 'index.md';
...
const getItemById = (rootDir: string) => (id: string, fields: MdxKeys = [], locale?: string) => {
  const fullPath = getFullPathWithLocale(
    join(
      rootDir,
      id
    ),
...
const getAllList = (rootDir: string) => (fields: MdxKeys = [], locale?: string) => {
...
export const getGroupedTagsInfo = (fields: MdxKeys = [], locale?: string) => {
  ...
  const groupedTagsInfo = allBlogs.reduce((grouped: Record<string, IMdx[]>, blog: IMdx) => {
